...
DDD は、業務が複雑なシステムで特に力を発揮します。

DDD の目的は、難しい設計パターンを使うことではありません。
...
- DDD はフォルダ構成ではない
- Entity は同一性を持つ
- Value Object は値そのものに意味がある
- Aggregate は整合性を守るまとまり
- Repository は保存・取得の抽象
- Domain Service は Entity に置けないドメインルールを扱う
- UseCase は処理の流れを管理する
- Infrastructure は技術詳細を担当する
- クリーンアーキテクチャは DDD を守る構造として相性がよい

---

### DDD を学ぶときの考え方

DDD を学ぶと、どうしても専門用語に意識が向きがちです。

しかし、本当に大切なのは用語を暗記することではありません。

以下を考えられるようになることが重要です。

> この業務ルールは、どこに書くのが自然か？

この問いに答えられるようになると、DDD の理解はかなり進んでいます。

最初から完璧なモデルを作る必要はありません。

業務を理解し、言葉を揃え、少しずつコードに反映していくことが DDD の第一歩です。
